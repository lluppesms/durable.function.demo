...
public partial class DurableTest
{
    private async Task<bool> Execute_SequentialTest()
    {
        ...
        try
        {
            Utilities.DisplayMessage($"\n{DateTime.Now:hh:mm:ss} Starting Sequential Test...", ConsoleColor.Yellow);
        ...
        {
            Utilities.DisplayErrorMessage($"{DateTime.Now:hh:mm:ss} Error running Sequential Test: {Utilities.GetExceptionMessage(ex)}");
            Utilities.DisplayCompletionMessage(timer);
            return false;
        }
    }
